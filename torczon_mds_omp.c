...
void mds(double *point, double *endpoint, int n, double *val, double eps, int maxfevals, int maxiter, double mu,
    ...
    *nf = 0;
    ...
    int local_nf = 0; // Temporary variable for reduction

    #pragma omp parallel for reduction(+:local_nf)
    for (i = 0; i < n + 1; i++) {
        fu[i] = f(&u[i * n], n);
        local_nf++;
    }

    *nf += local_nf; // Add the local count to the total count
    ...
    *val = fu[0];
    ...
    while (terminate == 0 && iter < maxiter) {
        ...
        while (found_better == 0) {
            ...
            if (found_better == 1) {
                local_nf = 0; // Reset local_nf for the new parallel region
                #pragma omp parallel for reduction(+:local_nf)
                for (i = 1; i < n + 1; i++) {
                    for (j = 0; j < n; j++) {
                        r[i * n + j] = u[0 * n + j] - (u[i * n + j] - u[0 * n + j]);
